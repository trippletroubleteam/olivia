...
def olivia():
    ...
    return {
        ...
        "sentence": text,
        "suggestion": generate_suggestion(depression_score)
        }
...
def generate_suggestion(percent):
    if percent == 0:
        response = generate_completion(congrarts_prompt)
    elif percent > 0 and percent < 100:
        response = generate_completion(help_prompt)
    elif percent > 100:
        response = generate_completion(danger_prompt)
